...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] pb-8">
      ...
      {/* Recent Activity */}
      <div className="px-4 mt-6 md:mt-8 mb-8 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-3 md:mb-4">
          <h2 className={`text-lg md:text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
        ...
        <Card className={`${isDarkMode ? "glass-effect" : "bg-white shadow-sm"} w-full`}>
          <CardBody className="p-4">
            ...
            {activeProjects.length > 0 ? (
                ...
                {activeProjects.map((project) => (
                  <div key={project.id} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-beamly-secondary/20 flex items-center justify-center flex-shrink-0">
                        <Icon 
                          icon={project.category === 'development' ? 'lucide:code' : 'lucide:palette'} 
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className="text-beamly-secondary font-medium">{project.progress || 0}%</p>
                      <div className="w-16 h-1 bg-white/20 rounded-full mt-1">
                        <div 
                          className="h-full bg-beamly-secondary rounded-full transition-all"
              ...
              <div className="text-center py-4">
                <p className={isDarkMode ? "text-gray-400 mb-3" : "text-gray-600 mb-3"}>
            ...
            <Button 
              ...
              variant="flat"
              onPress={() => navigate(userData?.userType === 'client' ? '/post-job' : '/looking-for-work')}
            >
              {userData?.userType === 'client' ? t('home.postNewJob') : t('home.findNewProjects')}
            </Button>
          </CardBody>
        </Card>
      </div>
